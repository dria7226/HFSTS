...
if( program == NULL ) printf("ERROR: no memory allocated for 'program'.\n");
ENTER_TO_CONTINUE

for(; loop < program_capacity*DATA_AT(SETTINGS, PROGRAM_CHUNK_SIZE); loop++)
{
  if( program[loop] == NULL) printf("ERROR: no memory allocated for 'program[%u]'.\n", loop);
else
...
if( data == NULL ) printf("ERROR: no memory allocated for 'data'.\n");
ENTER_TO_CONTINUE
...
for(; loop < data_capacity; loop++)
{
  ...
  if( data[loop] == NULL )
    printf("ERROR: no memory allocated for 'data[%u]'.\n", loop);
  else
  {
  DATA_TYPE inner_loop = 0;

  printf("data[%u].capacity = %u\n", loop, data[loop].capacity);
  ENTER_TO_CONTINUE
