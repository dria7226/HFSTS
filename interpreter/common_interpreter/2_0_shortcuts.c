...
#define CORE_INSTRUCTIONS_INFO_AT(name)MEMORY_AT(CORE_INSTRUCTIONS_INFO, name)

#define AT_HEAD_OFFSET(x)          MEMORY_AT(ARRAY_AT(head_index), INDEX_AT(head_index) + x)
#define ADVANCE_HEAD               INDEX_AT(head_index) += 1 + core_instructions_arguments[AT_HEAD_OFFSET(0)];
