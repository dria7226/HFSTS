...
DATA_TYPE insert_array(DATA_TYPE insert_to_address, DATA_TYPE insert_from_address)
{
  ...
  DATA_TYPE step = 1 - 2*(insert_from_address > insert_to_address);
...
DATA_TYPE allocate_memory()
{
  ...
  // CORE_INSTRUCTIONS_INFO array
  memory[CORE_INSTRUCTIONS_INFO].capacity = (NUMBER_OF_CORE_INSTRUCTIONS - 1) * NUMBER_OF_CORE_INSTRUCTIONS_INFO_ELEMENTS;
  memory[CORE_INSTRUCTIONS_INFO].data = load_core_instructions_info();

  if(memory[CORE_INSTRUCTIONS_INFO].data == MEMORY_ALLOCATION_FAILED)
  {
    #ifdef CLIT
    PRINT("Couldn't allocate memory for 'memory[CORE_INSTRUCTIONS_INFO].data.\n'",0,0,0)
    #endif
    return 1;
  }
